...
{
    public string CustNum { get; set; }   // PCFs use trimmed custnum
    ...
    public string DisplayCustName => CustName.Replace("&", "(and)");

    public string CustNameWithNum => $"{CustName} ({CustNum.Trim()})";

    // Computed property for the 7-character left-padded string
    public string PaddedCustNum =>
        // Trim leading/trailing spaces from CustNum, then pad to 7 characters
        CustNum?.Trim().PadLeft(7, '0') ?? "0000000";
